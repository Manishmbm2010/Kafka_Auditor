...
("tier","topic","hostname","unix time epoch"). Such service can be centrally hosted and dynamically scaled as per requirement & load.
...
According to me kafka auditor should directly insert the audit data in some highly available cassandra like database rather than relying on same infrastructure(kafka) which we want to audit ,  

# Audit Checker

## Approach 1:

* Step1: A consumer process should stream the audit messages from Kafka Audit Topic can put this audit data in some database like Apache Cassandra, oracle or sql. where a view can be created to read the aggregated data from database.
...
* Error: No SUM aggregate function with null  argument type exists!
* Need to check this error further
 

### Response to front end can be like this(just an example) , Multiple end point can be created to retrieve the detailed view of auditing the data.
...
* If database approach has to be chosen to push the audit data , then auditing to kafka can be kept ass fall back. It means if database went down we don't lose auditing , this will give us more flexibility. 

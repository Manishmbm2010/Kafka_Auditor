...
public class AuditorTest {
	...
	public void testA_checkNoOfMessage_in_auditMap_multipleUnixEpoch() throws Exception {
		...
		for (int i = 0; i < 10; i++) {
			auditor.audit(unixEpoch, hostname, topic, "consume");
		}
		mapKey = new MapKey(Math.floorDiv(unixEpoch, 60 * 1), topic, hostname, "consume");
		for (int i = 0; i < 20; i++) {
			auditor.audit(unixEpoch + 60, hostname, topic, "consume");
		}
		MapKey mapKey2 = new MapKey(Math.floorDiv(unixEpoch + 60, 60 * 1), topic, hostname, "consume");
